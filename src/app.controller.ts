...
export class AppController {
  ...
  async fraudcheckXP(@Body() fraudcheckXpDto: FraudCheckIdXpDto) {
    let xp:any = await this.metaplexService.findNftByPublic(fraudcheckXpDto.publicKey);
    let numberedXp = Number.parseInt(xp.value);
    let user:SolUserDto= await this.appService.updateUserSpent(fraudcheckXpDto.userId,fraudcheckXpDto.totalXp,numberedXp);
    let availableXp = numberedXp - user.spentXp;
    await this.appService.updateMerchOrder(fraudcheckXpDto,user.id);
    return {
      status:HttpStatus.OK,
      message:`You have spent ${fraudcheckXpDto.totalXp} xp and now you got ${availableXp} xp`,
      data:{
        availableXp:availableXp,
        ordersXp:fraudcheckXpDto.totalXp,
        userId:user.id,
