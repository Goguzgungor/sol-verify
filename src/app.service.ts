...
import { SolUserDto, FraudCheckXpDto, MerchDto } from './verifylogic/dto/checkXpDto';
...
export class AppService {
  constructor(private dbService: DbService) {}
   ...
   async findOrCreateUser(publicKey:string,totalXp:number){
    ...
    try{
      user =await this.dbService.solUser.findFirstOrThrow({where:{publicId:publicKey}})
      return user;
    }catch(e){
      let solUser: SolUserDto = new SolUserDto();
  ...
  async createMerchOrder(fraudCheckXpDto:FraudCheckXpDto,userId:number){
    let user:SolUserDto;
    let merchList :MerchDto[] = fraudCheckXpDto.merch;
  ...
  async updateMerchOrder(fraudCheckXpDto:FraudCheckXpDto,userId:number){
    let user:SolUserDto;
    await this.dbService.merchOrder.updateMany({where:{
      orderId:fraudCheckXpDto.orderId
    },data:{
